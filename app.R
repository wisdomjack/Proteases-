...
proteasesLegend <-c("Chymotripsin (F, W, Y)", 
                    "Trypsin (K, R)", 
                    "Elastase (A, G)", 
                    "CNBr (M)")
...
# This function traverse through peptide and save the indices of the cut sites and save it in a vector
get_cut_sites <- function(peptide, enzyme){
  cut_sites <- which(peptide %in% enzyme)
  
  # if statement that will only append that last amino acid into cut targets if cut target != cut site
...
digest <- function (peptide="MGAAMSPKR", enzyme="K"){  
  ...
  # Convrtes chars to upper case
  toupper(digestedPeptide)
}



# Set up the user interfase HTML page:
ui <- fluidPage(  
